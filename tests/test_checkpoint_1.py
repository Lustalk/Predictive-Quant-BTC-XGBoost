...
def test_checkpoint_1_complete():
    ...
    for timeframe, data in data_dict.items():
        ...
        print(
            f"   📅 Date range: {data['timestamp'].min().strftime('%Y-%m-%d %H:%M')} to {data['timestamp'].max().strftime('%Y-%m-%d %H:%M')}"
        )
        ...
        print(
            f"   📉 Recent change: {((data['close'].iloc[-1] / data['close'].iloc[-48] - 1) * 100):+.2f}% (last 48 periods)"
        )
    ...
    print(
        f"   💾 Memory: {aligned_data.memory_usage(deep=True).sum() / 1024**2:.1f} MB"
    )
    ...
    for timeframe in timeframes:
        ...
        if close_col in aligned_data.columns:
            ...
            if len(aligned_data) > periods_24h.get(timeframe, 24):
                old_price = aligned_data[close_col].iloc[
                    -periods_24h.get(timeframe, 24)
                ]
                price_changes[timeframe] = (
                    latest_prices[timeframe] / old_price - 1
                ) * 100
    ...
    for timeframe in timeframes:
        if timeframe in latest_prices:
            ...
            print(
                f"   {timeframe.upper()}: ${latest_prices[timeframe]:.2f} ({change:+.2f}% 24h)"
            )
    ...
    for timeframe, date_info in summary["date_ranges"].items():
        ...
        print(
            f"   {timeframe.upper()}: {duration.days} days ({date_info['count']} candles)"
        )
    ...
    key_columns = ["timestamp"] + [
        f"close_{tf}" for tf in timeframes if f"close_{tf}" in aligned_data.columns
    ]
