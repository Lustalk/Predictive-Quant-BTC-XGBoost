...
class TestTechnicalIndicators:
    ...
    def test_stochastic_calculation(self, sample_data):
        ...
        assert (stoch_data['Stoch_K'] >= 0).all() and (stoch_data['Stoch_K'] <= 100).all()
        assert (stoch_data['Stoch_D'] >= 0).all() and (stoch_data['Stoch_D'] <= 100).all()
    
    def test_williams_r_calculation(self, sample_data):
        """Test Williams %R calculation."""
        indicators = TechnicalIndicators()
        
        wr = indicators.williams_r(sample_data['high'], sample_data['low'], sample_data['close'])
    ...
    def test_adx_calculation(self, sample_data):
        ...
        adx_data = indicators.adx(sample_data['high'], sample_data['low'], sample_data['close'])
...
class TestFeatureEngine:
    ...
    def test_create_price_features(self, sample_data):
        ...
        expected_features = [
            'open', 'high', 'low', 'close', 'high_low_ratio', 
            'close_open_ratio', 'body_size', 'price_change'
        ]
    ...
    def test_create_volume_features(self, sample_data):
        ...
        ratio_features = [col for col in volume_features.columns if 'volume_ratio' in col]
    ...
    def test_create_time_features(self, sample_data):
        ...
        expected_features = [
            ...
            'is_weekend', 'is_asian_session', 'is_european_session'
        ]
        assert all(feature in time_features.columns for feature in expected_features)
        
        # Check cyclical features are in correct range
        assert (time_features['hour_sin'] >= -1).all() and (time_features['hour_sin'] <= 1).all()
        assert (time_features['hour_cos'] >= -1).all() and (time_features['hour_cos'] <= 1).all()
    ...
    def test_create_all_features(self, sample_data):
        ...
        nan_percentage = all_features.isna().sum().sum() / (len(all_features) * len(all_features.columns))
    ...
    def test_feature_selection(self, sample_data):
        ...
        np.random.seed(42)
        target = pd.Series(np.random.binomial(1, 0.5, len(features)), index=features.index)
        
        # Test feature selection
        selected_features = engine.select_features(features, target, method='mutual_info', top_k=20)
...
class TestIntegration:
    ...
    def realistic_data(self):
        ...
        cyclical = 0.1 * np.sin(np.linspace(0, 20*np.pi, 1000))  # Some cyclical patterns
    ...
    def test_end_to_end_pipeline(self, realistic_data):
        ...
        future_returns = realistic_data['close'].pct_change(4).shift(-4)  # 4-hour forward return
        ...
        assert (feature_stats.loc['std'] > 0).all()  # All features should have variation
