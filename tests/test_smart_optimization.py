...
def test_objective_functions():
    ...
    try:
        ...
        dates = pd.date_range(
            start=datetime.now() - timedelta(days=5), periods=100, freq="1h"
        )
        np.random.seed(42)
        ...
        print(
            f"  ✅ Objective functions working - Sharpe: {sharpe:.3f}, Win Rate: {win_rate:.3f}"
        )
        assert True, "Imports should succeed"
    except Exception as e:
...
def test_smart_optimizer():
    ...
    try:
        ...
        dates = pd.date_range(
            start=datetime.now() - timedelta(days=2), periods=50, freq="1h"
        )
        np.random.seed(42)
        ...
        config = OptimizationConfig(
            n_calls=5, n_initial_points=3, n_jobs=1  # Very few calls for quick test
        )
        ...
        assert len(result.best_params) == len(
            parameter_names
...
def test_market_regime_detector():
    ...
    try:
        ...
        dates = pd.date_range(
            start=datetime.now() - timedelta(days=10), periods=200, freq="1h"
        )
        np.random.seed(42)
        ...
        assert (
            regime_result.current_regime is not None
        ), "Should identify current regime"

        # Test regime-specific parameters
        regime_params = detector.get_regime_specific_parameters(
        ...
        print(
            f"  ✅ Market regime detector working - Current: {regime_result.current_regime.value}"
        )
        assert True, "Imports should succeed"
    except Exception as e:
...
def test_feature_importance_filter():
    ...
    try:
        ...
        print(
            f"  ✅ Feature importance filter working - {len(rankings)} indicators ranked"
        )
        assert True, "Imports should succeed"
    except Exception as e:
