    ...
    def get_trading_config(self) -> TradingConfig:
        ...
        if self._trading_config is None:
            ...
            if "app" in config_data:
                ...
                self._trading_config.app_name = app_config.get(
                    "name", self._trading_config.app_name
                )
                self._trading_config.app_version = app_config.get(
                    "version", self._trading_config.app_version
                )
                self._trading_config.environment = app_config.get(
                    "environment", self._trading_config.environment
                )
                self._trading_config.debug = app_config.get(
                    "debug", self._trading_config.debug
                )
