...
class XGBoostDiagnosticVisualizer:
    ...
    def create_comprehensive_diagnostic_dashboard(self,
        ...
        plt.suptitle('🔬 XGBoost Comprehensive Diagnostic Dashboard\n' + 
    ...
    def create_decision_boundary_analysis(self,
                                        model: xgb.XGBClassifier,
                                        X_train: pd.DataFrame,
                                        X_test: pd.DataFrame,
                                        y_train: pd.Series,
        ...
        self._plot_2d_decision_boundary(axes[0, 0], model, X_train, y_train, feature_names)
        
        # 2. Prediction Confidence Distribution (Top Middle)
        self._plot_confidence_distribution(axes[0, 1], y_proba_train, y_proba_test, y_train, y_test)
        ...
        self._plot_feature_contribution_heatmap(axes[1, 2], model, X_test, feature_names)
        ...
        plt.suptitle('🎯 XGBoost Decision Boundary & Behavior Analysis\n' +
    ...
    def create_feature_deep_dive(self,
                               model: xgb.XGBClassifier,
                               X_train: pd.DataFrame,
                               X_test: pd.DataFrame,
                               y_train: pd.Series,
        ...
        self._plot_feature_distribution_by_target(axes[1, 2], X_train, y_train, feature_names)
        self._plot_feature_outlier_impact(axes[1, 3], X_train, y_train, feature_names)
        
        # 3. Advanced Feature Insights (Bottom Row)
        self._plot_feature_interaction_strength(axes[2, 0], model, X_train, feature_names)
        self._plot_feature_prediction_contribution(axes[2, 1], model, X_test, feature_names)
        ...
        plt.suptitle('🔍 Feature Engineering Deep Dive Analysis\n' +
    ...
    def create_performance_optimization_insights(self,
                                               model: xgb.XGBClassifier,
                                               X_train: pd.DataFrame,
                                               X_test: pd.DataFrame,
                                               y_train: pd.Series,
        ...
        self._plot_overfitting_analysis(axes[0, 0], model, X_train, X_test, y_train, y_test)
        ...
        plt.suptitle('⚡ XGBoost Performance Optimization Insights\n' +
    ...
    def _plot_learning_curves(self, ax, model, X, y):
        ...
        try:
            ...
            ax.plot(train_sizes_abs, train_mean, 'o-', color=self.colors['primary'], 
                   label='Training Score', linewidth=2, markersize=6)
            ax.fill_between(train_sizes_abs, train_mean - train_std, train_mean + train_std, 
                           alpha=0.2, color=self.colors['primary'])
            
            ax.plot(train_sizes_abs, val_mean, 'o-', color=self.colors['danger'], 
            ...
            if gap > 0.1:
                ax.text(0.05, 0.95, f'⚠️ Overfitting detected\nGap: {gap:.3f}', 
                       transform=ax.transAxes, fontsize=10, 
                       bbox=dict(boxstyle="round,pad=0.3", facecolor='yellow', alpha=0.7))
        except Exception as e:
            ax.text(0.5, 0.5, f'Learning curves unavailable\n{str(e)[:50]}...', 
    ...
    def _plot_validation_curves(self, ax, X, y):
        ...
        try:
            ...
            train_scores, val_scores = validation_curve(
                xgb.XGBClassifier(random_state=42, verbosity=0), X, y,
            ...
            ax.plot(param_range, train_mean, 'o-', color=self.colors['primary'], 
                   label='Training Score', linewidth=2)
            ax.fill_between(param_range, train_mean - train_std, train_mean + train_std, 
                           alpha=0.2, color=self.colors['primary'])
            
            ax.plot(param_range, val_mean, 'o-', color=self.colors['danger'], 
            ...
            ax.axvline(optimal_depth, color=self.colors['success'], linestyle='--', 
                      label=f'Optimal Depth: {optimal_depth}')
            
            ax.set_xlabel('Max Depth')
            ax.set_ylabel('Accuracy Score')
            ax.set_title('🎛️ Hyperparameter Validation Curves', fontweight='bold', fontsize=12)
            ax.legend()
            ax.grid(True, alpha=0.3)
        except Exception as e:
            ax.text(0.5, 0.5, f'Validation curves unavailable\n{str(e)[:50]}...', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title('🎛️ Hyperparameter Validation Curves', fontweight='bold', fontsize=12)
    ...
    def _plot_prediction_confidence_analysis(self, ax, model, X_test, y_test):
        ...
        try:
            ...
            ax.plot(bin_centers, bin_accuracies, 'o-', color=self.colors['primary'], 
            ...
            for i, (x, y, count) in enumerate(zip(bin_centers, bin_accuracies, bin_counts)):
                if count > 0:
                    ax.annotate(f'{count}', (x, y), xytext=(5, 5), 
                              textcoords='offset points', fontsize=8)
            
            ax.set_xlabel('Mean Predicted Probability')
            ax.set_ylabel('Fraction of Positives')
            ax.set_title('🎯 Prediction Confidence Calibration', fontweight='bold', fontsize=12)
            ax.legend()
            ax.grid(True, alpha=0.3)
            
            # Calculate calibration error
            cal_error = np.mean(np.abs(np.array(bin_accuracies) - bin_centers))
            ax.text(0.05, 0.95, f'Calibration Error: {cal_error:.3f}', 
                   transform=ax.transAxes, fontsize=10,
                   bbox=dict(boxstyle="round,pad=0.3", facecolor='lightblue', alpha=0.7))
            
        except Exception as e:
            ax.text(0.5, 0.5, f'Confidence analysis unavailable\n{str(e)[:50]}...', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title('🎯 Prediction Confidence Analysis', fontweight='bold', fontsize=12)
    ...
    def _plot_feature_importance_evolution(self, ax, model, feature_names):
        ...
        try:
            ...
            top_features = sorted(importance_gain.items(), key=lambda x: x[1], reverse=True)[:10]
            ...
            if top_features:
                ...
                # Normalize values
                gains = np.array(gains) / max(gains) if max(gains) > 0 else np.array(gains)
                covers = np.array(covers) / max(covers) if max(covers) > 0 else np.array(covers)
                freqs = np.array(freqs) / max(freqs) if max(freqs) > 0 else np.array(freqs)
                
                x = np.arange(len(features))
                width = 0.25
                
                ax.bar(x - width, gains, width, label='Gain', color=self.colors['primary'], alpha=0.8)
                ax.bar(x, covers, width, label='Cover', color=self.colors['secondary'], alpha=0.8)
                ax.bar(x + width, freqs, width, label='Frequency', color=self.colors['success'], alpha=0.8)
                
                ax.set_xlabel('Features')
                ax.set_ylabel('Normalized Importance')
                ax.set_title('🔬 Feature Importance Evolution', fontweight='bold', fontsize=12)
                ax.set_xticks(x)
                ax.set_xticklabels([f[:10] + '...' if len(f) > 10 else f for f in features], rotation=45)
                ax.legend()
                ax.grid(True, alpha=0.3)
            else:
    ...
    def _plot_error_analysis(self, ax, model, X_test, y_test, feature_names):
        ...
        try:
            ...
            colors = [self.colors['success'], self.colors['warning'], self.colors['info'], self.colors['danger']]
            
            for i, (category, mask, color) in enumerate(zip(categories, masks, colors)):
                if mask.sum() > 0:
                    confidence = y_proba[mask]
                    ax.hist(confidence, bins=20, alpha=0.6, label=f'{category} ({mask.sum()})', 
                           color=color, density=True)
            
            ax.set_xlabel('Prediction Confidence')
            ax.set_ylabel('Density')
            ax.set_title('❌ Error Analysis by Confidence', fontweight='bold', fontsize=12)
            ax.legend()
            ax.grid(True, alpha=0.3)
            
            # Add error rates
            fp_rate = false_positives.sum() / (false_positives.sum() + true_negatives.sum()) if (false_positives.sum() + true_negatives.sum()) > 0 else 0
            fn_rate = false_negatives.sum() / (false_negatives.sum() + true_positives.sum()) if (false_negatives.sum() + true_positives.sum()) > 0 else 0
            
            ax.text(0.05, 0.95, f'FP Rate: {fp_rate:.3f}\nFN Rate: {fn_rate:.3f}', 
                   transform=ax.transAxes, fontsize=10,
                   bbox=dict(boxstyle="round,pad=0.3", facecolor='lightyellow', alpha=0.7))
            
        except Exception as e:
            ax.text(0.5, 0.5, f'Error analysis unavailable\n{str(e)[:50]}...', 
                   ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_model_calibration(self, ax, model, X_test, y_test):
        ...
        try:
            ...
            ax.text(0.5, 0.5, f'Calibration analysis unavailable\n{str(e)[:50]}...', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title('🌡️ Model Calibration Analysis', fontweight='bold', fontsize=12)
        ...
        try:
            ...
            if len(X_test) > 20:
                ...
                if volatility_feature:
                    ...
                    med_vol = (vol_values > vol_terciles[0]) & (vol_values <= vol_terciles[1])
                    ...
                    bars = ax.bar(regimes, accuracies, color=[self.colors['success'], 
                                 self.colors['warning'], self.colors['danger']], alpha=0.7)
                    ...
                    ax.set_title('📈 Performance Across Market Regimes', fontweight='bold', fontsize=12)
                    ax.grid(True, alpha=0.3)
                    ax.set_ylim(0, 1)
                else:
                    ax.text(0.5, 0.5, 'No volatility features found\nfor regime analysis', 
                           ha='center', va='center', transform=ax.transAxes)
            ...
            ax.text(0.5, 0.5, f'Market regime analysis unavailable\n{str(e)[:50]}...', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title('📈 Market Regime Performance', fontweight='bold', fontsize=12)
    ...
    def _plot_feature_interactions(self, ax, model, X_train, feature_names):
        ...
        try:
            ...
            if len(top_indices) > 1:
                ...
                ax.set_xticklabels([f[:8] + '...' if len(f) > 8 else f for f in top_features], rotation=45)
                ax.set_yticklabels([f[:8] + '...' if len(f) > 8 else f for f in top_features])
                ...
                ax.set_title('🔗 Feature Interactions (Correlation)', fontweight='bold', fontsize=12)
            ...
            ax.text(0.5, 0.5, f'Feature interaction analysis unavailable\n{str(e)[:50]}...', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title('🔗 Feature Interactions', fontweight='bold', fontsize=12)
    ...
    def _plot_hyperparameter_sensitivity(self, ax, X_train, y_train):
        ...
        try:
            ...
            for i, lr in enumerate(learning_rates):
                for j, depth in enumerate(max_depths):
                    try:
                        ...
                        model = xgb.XGBClassifier(
                            learning_rate=lr, max_depth=depth, n_estimators=50,
            ...
            ax.set_title('🎛️ Hyperparameter Sensitivity Analysis', fontweight='bold', fontsize=12)
            ax.set_xlabel('Max Depth')
            ...
            rect = patches.Rectangle((best_j-0.5, best_i-0.5), 1, 1, 
                                   linewidth=3, edgecolor='red', facecolor='none')
            ax.add_patch(rect)
            
        except Exception as e:
            ax.text(0.5, 0.5, f'Hyperparameter analysis unavailable\n{str(e)[:50]}...', 
                   ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_2d_decision_boundary(self, ax, model, X_train, y_train, feature_names):
        ...
        try:
            ...
            if len(top_2_indices) >= 2:
                ...
                feature_1, feature_2 = feature_names[top_2_indices[0]], feature_names[top_2_indices[1]]
                ...
                # Plot data points
                scatter = ax.scatter(X_2d.iloc[:, 0], X_2d.iloc[:, 1], c=y_train, 
                                   cmap='RdYlBu', edgecolors='black', alpha=0.8)
                
                ax.set_xlabel(feature_1[:15] + '...' if len(feature_1) > 15 else feature_1)
                ax.set_ylabel(feature_2[:15] + '...' if len(feature_2) > 15 else feature_2)
            ...
            ax.text(0.5, 0.5, f'Decision boundary unavailable\n{str(e)[:50]}...', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title('🎯 2D Decision Boundary', fontweight='bold', fontsize=12)
    
    def _plot_confidence_distribution(self, ax, y_proba_train, y_proba_test, y_train, y_test):
        """Plot confidence distribution for train and test sets."""
        try:
            ax.hist(y_proba_train, bins=30, alpha=0.6, label=f'Train (n={len(y_proba_train)})', 
                   color=self.colors['primary'], density=True)
            ax.hist(y_proba_test, bins=30, alpha=0.6, label=f'Test (n={len(y_proba_test)})', 
            ...
            ax.text(0.5, 0.5, f'Confidence distribution unavailable\n{str(e)[:50]}...', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title('📊 Confidence Distribution', fontweight='bold', fontsize=12)
    ...
    def _plot_prediction_truth_scatter(self, ax, y_proba_test, y_test):
        ...
        try:
            ...
            ax.scatter(y_proba_test, y_test_jitter, alpha=0.6, color=self.colors['primary'])
            ...
            ax.text(0.5, 0.5, f'Prediction scatter unavailable\n{str(e)[:50]}...', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title('🎯 Prediction vs Truth', fontweight='bold', fontsize=12)
    ...
    def _plot_prediction_uncertainty(self, ax, model, X_test, y_test):
        ...
        ax.text(0.5, 0.5, 'Prediction Uncertainty\nAnalysis', ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_feature_contribution_heatmap(self, ax, model, X_test, feature_names):
        ...
        ax.text(0.5, 0.5, 'Feature Contribution\nHeatmap', ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_temporal_decision_patterns(self, ax, y_proba_test, y_test):
        ...
        ax.text(0.5, 0.5, 'Temporal Decision\nPatterns', ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_boundary_stability(self, ax, model, X_test):
        ...
        ax.text(0.5, 0.5, 'Boundary Stability\nAnalysis', ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_confidence_evolution(self, ax, y_proba_test):
        ...
        ax.text(0.5, 0.5, 'Confidence Evolution\nOver Time', ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_feature_importance_by_type(self, ax, model, feature_names):
        ...
        try:
            ...
            # Categorize features
            categories = {
                'Technical': ['SMA', 'EMA', 'RSI', 'MACD', 'BB_', 'ATR', 'Stoch', 'ADX', 'VWAP', 'Williams', 'CCI', 'ROC', 'Momentum', 'OBV'],
                'Price': ['close', 'open', 'high', 'low', 'price', 'body', 'shadow', 'range'],
                'Volume': ['volume', 'OBV'],
                'Volatility': ['vol', 'atr'],
                'Time': ['hour', 'day', 'month', 'session', 'weekend'],
                'Statistical': ['mean', 'std', 'min', 'max', 'pct', 'lag', 'momentum', 'median', 'skew', 'kurt']
            }
            ...
            for i, feature_name in enumerate(feature_names):
                ...
                for cat, keywords in categories.items():
                    if any(keyword.lower() in feature_name.lower() for keyword in keywords):
                        feature_type_importance[cat] = feature_type_importance.get(cat, 0) + importance[i]
                        categorized = True
                        break
                if not categorized:
            ...
            bars = ax.bar(types, importances, color=[self.colors['primary'], self.colors['secondary'], 
                         self.colors['success'], self.colors['danger'], self.colors['warning'], 
                         self.colors['info']][:len(types)])
            ...
            ax.text(0.5, 0.5, f'Feature type analysis\nunavailable: {str(e)[:30]}...', 
    ...
    def _plot_feature_gain_vs_cover(self, ax, model, feature_names):
        ...
        try:
            ...
            if common_features:
                ...
                # Create scatter plot
                scatter = ax.scatter(gains, covers, alpha=0.7, c=range(len(gains)), 
                                   cmap='viridis', s=60)
                
                # Add feature labels for top features
                top_features = sorted(zip(common_features, gains, covers), 
                                    key=lambda x: x[1], reverse=True)[:5]
                
                for feature, gain, cover in top_features:
                    ax.annotate(feature[:8] + '...' if len(feature) > 8 else feature, 
                              (gain, cover), xytext=(5, 5), 
                ...
                ax.text(0.5, 0.5, 'No gain/cover data\navailable', 
                       ha='center', va='center', transform=ax.transAxes)
                ax.set_title('📈 Gain vs Cover', fontweight='bold', fontsize=10)
                
        except Exception as e:
            ax.text(0.5, 0.5, f'Gain vs cover analysis\nunavailable: {str(e)[:30]}...', 
                   ha='center', va='center', transform=ax.transAxes, fontsize=8)
            ax.set_title('📈 Gain vs Cover', fontweight='bold', fontsize=10)
    ...
    def _plot_feature_redundancy_analysis(self, ax, X_train, feature_names):
        ...
        try:
            ...
            if high_corr_pairs:
                ...
                ax.hist(all_correlations, bins=20, alpha=0.7, color=self.colors['primary'], 
                       edgecolor='black')
                ax.axvline(0.8, color=self.colors['danger'], linestyle='--', 
                ...
                # Add summary text
                redundancy_pct = len(high_corr_pairs) / (len(feature_names) * (len(feature_names) - 1) / 2) * 100
                ax.text(0.05, 0.95, f'Redundancy: {redundancy_pct:.1f}%', 
                       transform=ax.transAxes, fontsize=8,
                       bbox=dict(boxstyle="round,pad=0.3", facecolor='yellow', alpha=0.7))
            else:
                ax.text(0.5, 0.5, 'No high correlation\nfeatures found', 
                       ha='center', va='center', transform=ax.transAxes)
                ax.set_title('🔄 Redundancy Analysis', fontweight='bold', fontsize=10)
                
        except Exception as e:
            ax.text(0.5, 0.5, f'Redundancy analysis\nunavailable: {str(e)[:30]}...', 
                   ha='center', va='center', transform=ax.transAxes, fontsize=8)
            ax.set_title('🔄 Redundancy Analysis', fontweight='bold', fontsize=10)
    ...
    def _plot_feature_stability(self, ax, model, X_train, y_train, feature_names):
        ...
        try:
            ...
            for i in range(n_bootstrap):
                ...
                # Train simple model
                model_boot = xgb.XGBClassifier(n_estimators=50, max_depth=4, random_state=i, verbosity=0)
            ...
            if feature_importance_variations:
                ...
                stability_scores = 1 - (np.std(importance_array, axis=0) / (np.mean(importance_array, axis=0) + 1e-8))
                ...
                bars = ax.barh(range(len(top_10_stability)), top_10_stability, 
                              color=self.colors['success'], alpha=0.7)
                
                ax.set_yticks(range(len(top_10_names)))
                ax.set_yticklabels([f[:10] + '..' if len(f) > 10 else f for f in top_10_names], fontsize=8)
                ax.set_xlabel('Stability Score')
                ax.set_title('⚖️ Feature Stability', fontweight='bold', fontsize=10)
                ax.grid(True, alpha=0.3)
                
                # Add stability threshold
                ax.axvline(x=0.8, color=self.colors['danger'], linestyle='--', 
            ...
            ax.text(0.5, 0.5, f'Stability analysis\nunavailable: {str(e)[:30]}...', 
                   ha='center', va='center', transform=ax.transAxes, fontsize=8)
            ax.set_title('⚖️ Feature Stability', fontweight='bold', fontsize=10)
    ...
    def _plot_feature_selection_impact(self, ax, X_train, y_train, feature_names):
        ...
        try:
            ...
            for n_features in n_features_range:
                if n_features <= len(feature_names):
                    try:
                        ...
                        cv_scores = cross_val_score(model, X_selected, y_train, cv=3, scoring='accuracy')
            ...
            # Plot the impact
            ax.plot(n_features_range, scores, 'o-', color=self.colors['primary'], linewidth=2, markersize=6)
            
            # Find optimal number of features
            if scores:
                optimal_idx = np.argmax(scores)
                optimal_n = n_features_range[optimal_idx]
                ax.axvline(optimal_n, color=self.colors['success'], linestyle='--', 
            ...
            ax.text(0.5, 0.5, f'Selection impact\nunavailable: {str(e)[:30]}...', 
                   ha='center', va='center', transform=ax.transAxes, fontsize=8)
            ax.set_title('🎯 Selection Impact', fontweight='bold', fontsize=10)
    ...
    def _plot_feature_correlation_network(self, ax, X_train, feature_names):
        ...
        try:
            ...
            if len(top_10_features) > 1:
                ...
                # Plot as heatmap
                im = ax.imshow(subset_corr.values, cmap='coolwarm', aspect='auto', vmin=-1, vmax=1)
                
                # Set labels
                ax.set_xticks(range(len(top_10_features)))
                ax.set_yticks(range(len(top_10_features)))
                ax.set_xticklabels([f[:8] + '..' if len(f) > 8 else f for f in top_10_features], rotation=45, fontsize=8)
                ax.set_yticklabels([f[:8] + '..' if len(f) > 8 else f for f in top_10_features], fontsize=8)
            ...
            ax.text(0.5, 0.5, f'Correlation network\nunavailable: {str(e)[:30]}...', 
                   ha='center', va='center', transform=ax.transAxes, fontsize=8)
            ax.set_title('🕸️ Correlation Network', fontweight='bold', fontsize=10)
    ...
    def _plot_feature_distribution_by_target(self, ax, X_train, y_train, feature_names):
        ...
        try:
            ...
            if len(top_3_indices) > 0:
                ...
                ax.hist(class_0_values, bins=20, alpha=0.6, label='Class 0 (Down)', 
                       color=self.colors['danger'], density=True)
                ax.hist(class_1_values, bins=20, alpha=0.6, label='Class 1 (Up)', 
                       color=self.colors['success'], density=True)
                
                ax.set_xlabel('Feature Value')
                ax.set_ylabel('Density')
                ax.set_title(f'📊 {feature_name[:15]}...', fontweight='bold', fontsize=10)
                ax.legend(fontsize=8)
                ax.grid(True, alpha=0.3)
            ...
            ax.text(0.5, 0.5, f'Distribution analysis\nunavailable: {str(e)[:30]}...', 
                   ha='center', va='center', transform=ax.transAxes, fontsize=8)
            ax.set_title('📊 Distribution by Target', fontweight='bold', fontsize=10)
    ...
    def _plot_feature_outlier_impact(self, ax, X_train, y_train, feature_names):
        ...
        try:
            ...
            outlier_percentages = []
            feature_subset = X_train.iloc[:, :min(10, len(feature_names))]  # Top 10 features
            ...
            for col in feature_subset.columns:
                ...
                outliers = (feature_subset[col] < lower_bound) | (feature_subset[col] > upper_bound)
                outlier_pct = outliers.mean() * 100
                outlier_percentages.append(outlier_pct)
            
            if outlier_percentages:
                # Create bar plot
                feature_subset_names = [f[:8] + '..' if len(f) > 8 else f for f in feature_subset.columns]
                bars = ax.bar(range(len(outlier_percentages)), outlier_percentages, 
                ...
                # Add threshold line
                ax.axhline(y=5, color=self.colors['danger'], linestyle='--', 
                          alpha=0.7, label='5% threshold')
                ax.legend(fontsize=8)
            else:
                ax.text(0.5, 0.5, 'No outlier data\navailable', 
                       ha='center', va='center', transform=ax.transAxes)
                ax.set_title('⚡ Outlier Impact', fontweight='bold', fontsize=10)
                
        except Exception as e:
            ax.text(0.5, 0.5, f'Outlier analysis\nunavailable: {str(e)[:30]}...', 
                   ha='center', va='center', transform=ax.transAxes, fontsize=8)
            ax.set_title('⚡ Outlier Impact', fontweight='bold', fontsize=10)
    
    def _plot_feature_interaction_strength(self, ax, model, X_train, feature_names):
        """Plot feature interaction strength."""
        ax.text(0.5, 0.5, 'Feature Interaction\nStrength', ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_feature_prediction_contribution(self, ax, model, X_test, feature_names):
        ...
        ax.text(0.5, 0.5, 'Feature Prediction\nContribution', ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_feature_temporal_importance(self, ax, X_train, feature_names):
        ...
        ax.text(0.5, 0.5, 'Feature Temporal\nImportance', ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_regularization_impact(self, ax, X_train, y_train):
        ...
        ax.text(0.5, 0.5, 'Regularization\nImpact', ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_training_efficiency(self, ax, model):
        ...
        ax.text(0.5, 0.5, 'Training\nEfficiency', ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_prediction_speed_analysis(self, ax, model, X_test):
        ...
        ax.text(0.5, 0.5, 'Prediction Speed\nAnalysis', ha='center', va='center', transform=ax.transAxes)
    ...
    def _plot_improvement_recommendations(self, ax, model, X_train, y_train):
        ...
        ax.text(0.5, 0.5, 'Improvement\nRecommendations', ha='center', va='center', transform=ax.transAxes)
