...
class OptimizationIntegration:
    ...
    def run_comprehensive_optimization(
        self, data: pd.DataFrame, save_results: bool = True, update_config: bool = True
    ) -> ComprehensiveOptimizationResult:
        """
        ...
        # Run optimization
        result = optimizer.optimize_all_indicators(data=data, validation_split=0.2)

        self.logger.info(
            f"✅ Optimization complete! Best score: {result.best_overall_score:.3f}"
        )
    ...
    def apply_optimized_parameters(
        ...
        if optimal_tp_sl:
            current_config["strategy"]["risk_management"]["stop_loss"] = (
                optimal_tp_sl.get("stop_loss", 0.05)
            )
            current_config["strategy"]["risk_management"]["take_profit"] = (
                optimal_tp_sl.get("take_profit", 0.10)
            )

            # Add new optimized parameters
            current_config["strategy"]["risk_management"]["risk_reward_ratio"] = (
                optimal_tp_sl.get("risk_reward_ratio", 2.0)
            )
            current_config["strategy"]["risk_management"]["position_timeout_hours"] = (
                optimal_tp_sl.get("position_timeout", 24)
            )
    ...
    def _save_optimization_results(self, result: ComprehensiveOptimizationResult):
        ...
        try:
            results_dict = {
                ...
                "indicator_scores": {
                    ind.name: ind.importance_score for ind in result.indicator_rankings
                },
                "indicator_details": {
                    ind.name: {
    ...
    def _update_configuration_with_results(
        self, result: ComprehensiveOptimizationResult
    ):
    ...
    def get_optimization_summary(self) -> Optional[Dict[str, Any]]:
        ...
        summary = {
            ...
            # TP/SL Summary
            "risk_management": {
                "stop_loss_percent": results.get("optimal_tp_sl", {}).get(
                    "stop_loss", 0.05
                )
                * 100,
                "take_profit_percent": results.get("optimal_tp_sl", {}).get(
                    "take_profit", 0.10
                )
                * 100,
                "risk_reward_ratio": results.get("optimal_tp_sl", {}).get(
            ...
            "vs_default_config": {
                "stop_loss_change": (
                    results.get("optimal_tp_sl", {}).get("stop_loss", 0.05) - 0.05
                )
                * 100,
                "take_profit_change": (
                    results.get("optimal_tp_sl", {}).get("take_profit", 0.10) - 0.10
                )
                * 100,
                "risk_reward_change": results.get("optimal_tp_sl", {}).get(
                    "risk_reward_ratio", 2.0
                )
                - 2.0,
            },
...
def main():
    ...
    if existing_results:
        ...
        if summary:
            ...
            print(
                f"Take Profit: {summary['risk_management']['take_profit_percent']:.1f}%"
            )
