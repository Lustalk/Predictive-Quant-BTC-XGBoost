...
class FeatureImportanceFilter:
    ...
    def analyze_feature_importance(
        self, data: pd.DataFrame, target_column: str = None, prediction_horizon: int = 1
        ...
        importance_results = self._run_multiple_importance_analyses(
        ...
        self.logger.info(
            f"Selected {len(final_features)} features from {len(features.columns)} total"
        )
        self.logger.info(f"Removed {len(redundant_features)} redundant features")

        return result

    def filter_indicator_parameters(
        self, data: pd.DataFrame, all_parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for indicator, importance in indicator_importance.items():
            if importance >= importance_threshold:
                ...
                indicator_params = self._get_indicator_parameters(
                    indicator, all_parameters
                )
                filtered_parameters.update(indicator_params)

                self.logger.info(
                    f"Including {indicator} (importance: {importance:.4f})"
                )
            else:
                self.logger.info(
                    f"Excluding {indicator} (importance: {importance:.4f})"
                )

        self.logger.info(
            f"Filtered parameters: {len(filtered_parameters)} from {len(all_parameters)}"
        )
    ...
    def _create_price_direction_target(
    ...
    def _align_features_target(
        ...
        try:
            ...
            self.logger.info(
                f"Aligned data: {len(aligned_features)} samples, {len(aligned_features.columns)} features"
            )
    ...
    def _run_multiple_importance_analyses(
        self, features: pd.DataFrame, target: pd.Series, target_name: str
    ...
    def _combine_importance_scores(
        ...
        for feature in all_features:
            ...
            for method, method_scores in importance_results.items():
                if feature in method_scores:
                    ...
                    max_score = (
                        max(method_scores.values()) if method_scores.values() else 1.0
                    )
                    normalized_score = (
                        method_scores[feature] / max_score if max_score > 0 else 0.0
                    )
        ...
        max_combined = (
            max(combined_scores.values()) if combined_scores.values() else 1.0
        )
        if max_combined > 0:
    ...
    def _categorize_features(self, features: List[str]) -> Dict[str, List[str]]:
        ...
        for feature in features:
            ...
            if any(
                ...
                for keyword in ["price", "close", "open", "high", "low", "return"]
            ):
                ...
                keyword in feature_lower
                for keyword in ["volatility", "atr", "std", "bb_"]
            ):
                categories["volatility"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["rsi", "stoch", "williams", "cci"]
            ):
                categories["oscillators"].append(feature)
            elif any(
                keyword in feature_lower for keyword in ["macd", "momentum", "roc"]
            ):
                categories["momentum"].append(feature)
            elif any(
                keyword in feature_lower for keyword in ["sma", "ema", "ma_", "trend"]
            ):
                categories["trend"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["hour", "day", "month", "session"]
            ):
        ...
        try:
            ...
            for train_idx, test_idx in tscv.split(features):
                ...
                for feature, importance in zip(
                    features.columns, rf.feature_importances_
                ):
            ...
            for feature, importances in feature_importance_across_folds.items():
                if len(importances) > 1:
                    ...
                    if mean_importance > 0:
                        ...
                        stability = 1.0 / (
                            1.0 + cv
                        )  # Higher stability = lower variation
                    else:
                        stability = 0.0
    ...
    def _evaluate_feature_subset_performance(
        self, features: pd.DataFrame, target: pd.Series
    ) -> Dict[str, float]:
        ...
        try:
            ...
            rf_scores = cross_val_score(
                rf, features, target, cv=tscv, scoring="accuracy"
            )
            ...
            if HAS_XGBOOST:
                ...
                xgb_scores = cross_val_score(
                    xgb_model, features, target, cv=tscv, scoring="accuracy"
                )
                performance["xgboost_accuracy"] = xgb_scores.mean()
                performance["xgboost_std"] = xgb_scores.std()

            self.logger.info(
                f"Feature subset performance - RF: {performance['random_forest_accuracy']:.4f}"
            )
    ...
    def _map_features_to_indicators(
    ...
    def _get_indicator_parameters(
        self, indicator: str, all_parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
