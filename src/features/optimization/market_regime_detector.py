...
class MarketRegimeDetector:
    ...
    def detect_regimes(self, data: pd.DataFrame) -> RegimeDetectionResult:
        ...
        self.logger.info(
            f"Regime detection completed. Current regime: {current_regime.value}"
        )
        return result
    ...
    def predict_regime_transitions(
        self, current_data: pd.DataFrame, forecast_periods: int = 5
        ...
        try:
            ...
            for _ in range(forecast_periods):
                ...
                current_regime_probs = self.hmm_model.predict_proba(current_features)[
                    -1
                ]
                ...
                # Map to regime enum
                predicted_regime = list(MarketRegime)[
                    predicted_regime_idx % len(MarketRegime)
                ]
    ...
    def _calculate_regime_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            features["volatility_percentile"] = (
                features["volatility"].rolling(100).rank(pct=True)
            )
            ...
            features["volatility_regime"] = (
                features["volatility_percentile"] > 0.7
            ).astype(int)
            features["trend_regime"] = (
                np.abs(features["trend_strength"]) > 0.02
            ).astype(int)
    ...
    def _simple_regime_detection(self, features: pd.DataFrame) -> pd.Series:
        ...
        try:
            if (
                "volatility_percentile" in features.columns
                and "trend_strength" in features.columns
            ):
    ...
    def _map_labels_to_regimes(
        self, labels: pd.Series, features: pd.DataFrame
    ) -> pd.Series:
        ...
        try:
            ...
            for label in labels.unique():
                ...
                avg_volatility = (
                    label_features["volatility_percentile"].mean()
                    if "volatility_percentile" in label_features
                    else 0.5
                )
                avg_trend = (
                    label_features["trend_strength"].mean()
                    if "trend_strength" in label_features
                    else 0.0
                )
    ...
    def _calculate_regime_probabilities(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.gmm_model = GaussianMixture(
                n_components=n_regimes, random_state=42, max_iter=100
            )
            ...
            prob_df = pd.DataFrame(
                probabilities, index=features.index, columns=regime_names
            )
    ...
    def _analyze_regime_characteristics(
        self, data: pd.DataFrame, regimes: pd.Series, features: pd.DataFrame
        ...
        for regime in regimes.unique():
            ...
            try:
                ...
                volatility_percentile = (
                    regime_features["volatility_percentile"].mean()
                    if "volatility_percentile" in regime_features
                    else 0.5
                )
                trend_strength = (
                    abs(regime_features["trend_strength"].mean())
                    if "trend_strength" in regime_features
                    else 0.0
                )
                momentum_strength = (
                    abs(regime_features["rsi_momentum"].mean())
                    if "rsi_momentum" in regime_features
                    else 0.0
                )
