...
class WalkForwardAnalyzer:
    ...
    def run_walk_forward_analysis(
        ...
        for i, (
            ...
            self.logger.info(
                f"Processing period {i+1}/{len(periods)}: "
            ...
            try:
                ...
                test_score = objective_function(
                    test_data, optimization_result.best_params
                )
        ...
        self.logger.info(
            f"Average degradation: {analysis_results.average_degradation:.2%}"
        )
    ...
    def analyze_parameter_stability(
        ...
        for param_name, values in all_params.items():
            ...
            if len(values) > 2:
                ...
                trend_strength = (
                    abs(trend_slope) / values.std() if values.std() > 0 else 0
                )
            else:
                trend_slope = 0
                trend_strength = 0
    ...
    def find_robust_parameters(
        ...
        for param_name, stats in param_stats.items():
            if stats["stability"] >= stability_threshold:
                ...
                robust_params[param_name] = stats["median"]
                self.logger.warning(
                    f"Parameter {param_name} has low stability: {stats['stability']:.3f}"
                )
    ...
    def _generate_periods(self, data: pd.DataFrame, max_periods: int) -> List[Tuple]:
        ...
        while period_count < max_periods:
            ...
            periods.append(
                (train_data, test_data, train_start, train_end, test_start, test_end)
            )
    ...
    def _analyze_walk_forward_results(
        self, periods: List[WalkForwardPeriod], parameter_names: List[str]
    ) -> WalkForwardResults:
        ...
        if len(in_sample_scores) > 2:
            overall_stability = np.corrcoef(in_sample_scores, out_of_sample_scores)[
                0, 1
            ]
        ...
        for param_name in parameter_names:
            ...
            if len(param_values) > 1:
                ...
                cv = (
                    np.std(param_values) / np.abs(np.mean(param_values))
        ...
        if periods:
            ...
            for period in periods:
                weight = max(
                    0, period.out_of_sample_score
                )  # Use out-of-sample score as weight
                total_weight += weight
        ...
        performance_summary = {
            ...
            "best_out_of_sample": (
                max(out_of_sample_scores) if out_of_sample_scores else 0
            ),
            "worst_out_of_sample": (
                min(out_of_sample_scores) if out_of_sample_scores else 0
            ),
    ...
    def save_walk_forward_report(
        self, results: WalkForwardResults, save_path: str = "walk_forward_analysis"
    ) -> None:
        ...
        try:
            ...
            axes[0, 0].plot(
                periods, out_of_sample, "r-o", label="Out-of-sample", alpha=0.7
            )
            ...
            param_names = list(results.parameter_consistency.keys())[
                :10
            ]  # Top 10 parameters
            param_consistency = [
                results.parameter_consistency[name] for name in param_names
            ]
            ...
            values = [
                ...
                results.performance_summary["positive_periods"]
                / results.performance_summary["total_periods"],
            ]
            ...
            self.logger.info(
                f"Walk-forward analysis report saved to {save_path}_report.png"
            )
