...
class ObjectiveFunctions:
    ...
    def _generate_signals(
        ...
        try:
            ...
            target = self._create_target_variable(
                data, lookforward_periods, target_threshold
            )
            ...
            try:
                ...
                self.logger.warning(
                    "XGBoost not available, falling back to simple momentum strategy"
                )
    ...
    def _create_ml_features(
        ...
        try:
            ...
            features["bb_squeeze"] = (
                bb_data["BB_Width"] < bb_data["BB_Width"].rolling(20).mean() * 0.5
            ).astype(int)
            ...
            features["atr"] = self.technical_indicators.atr(
            ...
            features["price_above_sma5"] = (data["close"] > features["sma_5"]).astype(
                int
            )
            features["price_above_sma20"] = (data["close"] > features["sma_20"]).astype(
                int
            )
            ...
            features["trend_strength"] = (
                features["higher_high"] + features["higher_low"]
            )
    ...
    def _create_target_variable(
        self, data: pd.DataFrame, lookforward_periods: int = 4, threshold: float = 0.001
    ) -> pd.Series:
        ...
        try:
            ...
            future_returns = (
                data["close"]
                .pct_change(periods=lookforward_periods)
                .shift(-lookforward_periods)
            )
    ...
    def _fallback_momentum_strategy(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> pd.Series:
    ...
    def calculate_performance_metrics(
        ...
        try:
            ...
            metrics = {
                ...
                "max_drawdown": -self.max_drawdown_objective(
                    data, params
                ...
                "total_return": (
                    ((1 + returns).cumprod().iloc[-1] - 1) if len(returns) > 0 else 0.0
                ),
