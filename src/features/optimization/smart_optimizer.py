...
class SmartOptimizer:
    ...
    def optimize_indicators(
        ...
        """
        self.logger.info(
            f"Starting Bayesian optimization with {self.config.n_calls} evaluations"
        )
        ...
        def objective(**params):
            ...
            try:
                ...
                if score > self.best_score:
                    ...
                    self.logger.info(
                        f"New best score: {score:.6f} at evaluation {self.evaluation_count}"
                    )

                # Early stopping check
                if self._should_stop_early():
                    self.logger.info(
                        f"Early stopping triggered at evaluation {self.evaluation_count}"
                    )
                    return score
                ...
                if self.evaluation_count % 10 == 0:
                    ...
                    self.logger.info(
                        f"Evaluation {self.evaluation_count}/{self.config.n_calls}, "
        ...
        try:
            ...
            self.logger.info(
                f"Total evaluations: {optimization_result.evaluation_count}"
            )
    ...
    def parallel_regime_optimization(
        self,
        data: pd.DataFrame,
        ...
        self.logger.info(
            f"Found {len(unique_regimes)} market regimes: {unique_regimes}"
        )
        ...
        def optimize_regime(regime_name):
            ...
            if len(regime_data) < 50:  # Skip if insufficient data
                self.logger.warning(
                    f"Insufficient data for regime {regime_name}: {len(regime_data)} samples"
                )
                return regime_name, None

            self.logger.info(
                f"Optimizing regime '{regime_name}' with {len(regime_data)} samples"
            )
        ...
        self.logger.info(
            f"Completed regime optimization for {len(regime_results)} regimes"
        )
    ...
    def _should_stop_early(self) -> bool:
        ...
        recent_scores = self.optimization_history[
        ...
        if len(self.optimization_history) > self.config.early_stopping_patience:
            older_max = max(
                self.optimization_history[: -self.config.early_stopping_patience]
            )
            improvement = (
                (max_recent - older_max) / abs(older_max) if older_max != 0 else 0
            )
    ...
    def _calculate_parameter_importance(
        self, result: Any, parameter_names: List[str]
    ) -> Dict[str, float]:
        ...
        try:
            ...
            if total_importance > 0:
                ...
                importance = {
                    name: 1.0 / len(parameter_names) for name in parameter_names
                }
    ...
    def save_optimization_plots(
        self, result: OptimizationResult, save_path: str = "optimization_results"
    ):
