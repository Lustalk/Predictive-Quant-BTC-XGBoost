...
class ComprehensiveHyperparameterOptimizer:
    ...
    def __init__(self, 
        ...
        self.max_optimization_time = max_optimization_time_hours * 3600  # Convert to seconds
    ...
    def optimize_all_indicators(self, 
                               data: pd.DataFrame,
                               validation_split: float = 0.2) -> ComprehensiveOptimizationResult:
        """
        ...
        self.logger.info(f"Data: {len(data)} samples, Validation split: {validation_split:.1%}")
        ...
        self.logger.info(f"Train: {len(train_data)} samples, Validation: {len(val_data)} samples")
        ...
        combined_result = self._optimize_combined_strategy(train_data, val_data, individual_results)
        
        # Phase 3: Feature selection and final optimization
        self.logger.info("PHASE 3: Feature Selection & Final Optimization")
        final_result = self._final_optimization_with_selection(train_data, val_data, combined_result)
    ...
    def _test_individual_indicators(self, 
                                  train_data: pd.DataFrame, 
                                  val_data: pd.DataFrame) -> Dict[str, IndicatorPerformance]:
        ...
        indicator_tests = {
            ...
            'Bollinger_Bands': self.parameter_spaces.get_bollinger_bands_space(extended=True),
            'Moving_Averages': self.parameter_spaces.get_moving_averages_space(extended=True),
        ...
        for indicator_name, (search_space, param_names) in indicator_tests.items():
            ...
            optimizer = SmartOptimizer(OptimizationConfig(n_calls=50))  # Fewer calls per indicator
            
            def indicator_objective(data_subset, params):
                return self._evaluate_single_indicator_strategy(data_subset, params, indicator_name)
            ...
            try:
                ...
                performance = self._calculate_indicator_performance(
                    train_data, val_data, result.best_params, indicator_name, 
                ...
                self.logger.info(f"COMPLETED {indicator_name}: Sharpe={performance.sharpe_ratio:.3f}, "
    ...
    def _optimize_combined_strategy(self, 
                                  train_data: pd.DataFrame, 
                                  val_data: pd.DataFrame,
                                  individual_results: Dict) -> Dict[str, Any]:
        ...
        self.logger.info(f"Combining top indicators: {[ind.name for ind in top_indicators]}")
        
        # Create comprehensive search space
        search_space, param_names = self._create_comprehensive_search_space(top_indicators)
    ...
    def _final_optimization_with_selection(self, 
                                         train_data: pd.DataFrame, 
                                         val_data: pd.DataFrame,
                                         combined_params: Dict[str, Any]) -> ComprehensiveOptimizationResult:
        ...
        feature_importance = self._calculate_feature_importance(final_result, selected_features)
    ...
    def _create_comprehensive_search_space(self, top_indicators: List[IndicatorPerformance]) -> Tuple[List[Any], List[str]]:
        ...
        for indicator in top_indicators:
            if indicator.name == 'RSI':
                ...
                space, names = self.parameter_spaces.get_bollinger_bands_space(extended=False)
            elif indicator.name == 'Moving_Averages':
                space, names = self.parameter_spaces.get_moving_averages_space(extended=False)
            elif indicator.name == 'Stochastic':
                space, names = self.parameter_spaces.get_stochastic_space(extended=False)
        ...
        param_names.extend([
            ...
            'max_position_size', 'position_timeout', 'max_daily_trades',
            'min_confidence', 'min_volatility', 'spread_filter',
            'volatility_multiplier', 'trend_strength_filter'
        ])
        
        return search_space, param_names
    
    def _evaluate_single_indicator_strategy(self, 
                                          data: pd.DataFrame, 
                                          params: Dict[str, Any], 
        ...
        try:
            ...
            score = self._calculate_multi_objective_score(data, signals_with_tpsl, params)
    ...
    def _evaluate_combined_strategy(self, 
                                  data: pd.DataFrame, 
                                  params: Dict[str, Any], 
                                  indicators: List[IndicatorPerformance]) -> float:
        ...
        try:
            ...
            for indicator in indicators:
                indicator_signals = self._generate_indicator_signals(data, params, indicator.name)
    ...
    def _evaluate_final_strategy(self, 
                               data: pd.DataFrame, 
                               params: Dict[str, Any], 
    ...
    def _apply_advanced_tp_sl_logic(self, 
                                   data: pd.DataFrame, 
                                   signals: pd.Series, 
                                   params: Dict[str, Any]) -> pd.Series:
        ...
        for i in range(1, len(data)):
            ...
            if signals.iloc[i] != 0 and current_position == 0:
                ...
                vol_adjusted_stop = base_stop_loss * (1 + current_vol * volatility_multiplier)
    ...
    def _calculate_comprehensive_score(self, 
        ...
        try:
            ...
            avg_trade_return = returns[returns != 0].mean() if len(returns[returns != 0]) > 0 else 0
            
            # Penalty for too many or too few trades
            trade_penalty = 1.0
            if num_trades < 10:  # Too few trades
                trade_penalty = 0.5
            elif num_trades > len(data) * 0.1:  # Too many trades (more than 10% of periods)
                trade_penalty = 0.7
    ...
    def _calculate_indicator_performance(self, 
                                       train_data: pd.DataFrame,
                                       val_data: pd.DataFrame,
                                       params: Dict[str, Any],
                                       indicator_name: str,
                                       train_score: float,
                                       val_score: float) -> IndicatorPerformance:
        ...
        try:
            ...
            returns = self._calculate_strategy_returns(val_data, signals_with_tpsl, params)
            ...
            parameter_sensitivity = abs(train_score - val_score) / max(train_score, 0.001)
            ...
            self.logger.warning(f"Error calculating performance for {indicator_name}: {e}")
            return IndicatorPerformance(
                name=indicator_name,
                sharpe_ratio=0.0,
                max_drawdown=1.0,
    ...
    def _select_optimal_features(self) -> List[str]:
        ...
        threshold = 0.1
        selected_indicators = [
            perf.name for perf in self.indicator_performances 
            if perf.importance_score > threshold
        ]
        
        self.logger.info(f"🎯 Selected {len(selected_indicators)} indicators above threshold {threshold}")
        
        return selected_indicators
    
    def _create_final_search_space(self, selected_features: List[str]) -> Tuple[List[Any], List[str]]:
        ...
        param_names.extend([
            'stop_loss', 'take_profit', 'risk_reward_ratio',
            'max_position_size', 'position_timeout', 'min_confidence',
            'volatility_multiplier'
        ])
        
        return search_space, param_names
    
    def _generate_optimized_signals(self, 
                                  data: pd.DataFrame, 
                                  params: Dict[str, Any], 
                                  selected_features: List[str]) -> pd.Series:
        ...
        for feature in selected_features:
            ...
            indicator_perf = next((p for p in self.indicator_performances if p.name == feature), None)
    ...
    def _calculate_feature_importance(self, 
        ...
        trading_params = ['stop_loss', 'take_profit', 'risk_reward_ratio', 'position_timeout']
