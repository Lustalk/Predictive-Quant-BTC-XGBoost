...
class TechnicalIndicators:
    ...
    def ema(
        self, data: pd.Series, window: int, alpha: Optional[float] = None
    ) -> pd.Series:
        """
    ...
    def bollinger_bands(
        self, data: pd.Series, window: int = 20, std_dev: float = 2.0
    ) -> pd.DataFrame:
        """
        Bollinger Bands.
    ...
    def stochastic(
        ...
        return pd.DataFrame(
            {"Stoch_K": k_percent, "Stoch_D": d_percent}, index=close.index
        )
    ...
    def adx(
        ...
        plus_di = 100 * (
            plus_dm.rolling(window=window, min_periods=1).mean() / atr_values
        )
        minus_di = 100 * (
            minus_dm.rolling(window=window, min_periods=1).mean() / atr_values
        )
        ...
        return pd.DataFrame(
            {"ADX": adx, "Plus_DI": plus_di, "Minus_DI": minus_di}, index=close.index
        )
