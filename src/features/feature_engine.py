...
class FeatureEngine:
    ...
    def create_price_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in [1, 2, 3, 5, 10, 20]:
            ...
            features[f"log_return_{period}d"] = np.log(
    ...
    def create_volume_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in [5, 10, 20, 50]:
            ...
            features[f"volume_std_{window}d"] = (
                data["volume"].rolling(window).std() / vol_mean
            )

        # Price-volume relationships
        features["volume_price_trend"] = (
            (data["close"].pct_change() * data["volume"]).rolling(5).mean()
        )
        ...
        for period in [5, 10, 20]:
            features[f"volume_momentum_{period}d"] = data["volume"] / data[
                "volume"
            ].shift(period)
        ...
        features["vwap_deviation"] = (
            data["close"]
            - self.technical_indicators.vwap(
    ...
    def create_volatility_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in [5, 10, 20, 50, 100]:
            ...
            features[f"hist_vol_{window}d"] = returns.rolling(window).std() * np.sqrt(
                252
            )  # Annualized
        ...
        features["vol_ratio_short_long"] = (
            features["hist_vol_5d"] / features["hist_vol_50d"]
        )
        features["vol_ratio_mid_long"] = (
            features["hist_vol_20d"] / features["hist_vol_50d"]
        )

        # Volatility percentiles
        for window in [50, 100, 200]:
            vol_20 = features["hist_vol_20d"]
            features[f"vol_percentile_{window}d"] = vol_20.rolling(window).rank(
                pct=True
            )
    ...
    def create_lag_features(
        self, data: pd.DataFrame, lags: List[int] = [1, 2, 3, 6, 12, 24]
    ) -> pd.DataFrame:
        """
    ...
    def create_rolling_statistics(
        self, data: pd.DataFrame, windows: List[int] = [5, 10, 20, 50]
    ) -> pd.DataFrame:
        """
        ...
        for col in key_columns:
            if col in data.columns:
                for window in windows:
                    rolling_features[f"{col}_mean_{window}"] = (
                    ...
                    rolling_features[f"{col}_median_{window}"] = (
                        data[col].rolling(window).median()
                    )
                    rolling_features[f"{col}_skew_{window}"] = (
                        data[col].rolling(window).skew()
                    )
                    rolling_features[f"{col}_kurt_{window}"] = (
                        data[col].rolling(window).kurt()
                    )

                    # Percentile features
                    rolling_features[f"{col}_pct25_{window}"] = (
                        data[col].rolling(window).quantile(0.25)
                    )
                    rolling_features[f"{col}_pct75_{window}"] = (
                        data[col].rolling(window).quantile(0.75)
                    )

                    # Position in range
                    rolling_min = data[col].rolling(window).min()
                    rolling_max = data[col].rolling(window).max()
                    rolling_features[f"{col}_position_{window}"] = (
                        data[col] - rolling_min
                    ) / (rolling_max - rolling_min)

        self.logger.info(
            f"Created {len(rolling_features.columns)} rolling statistics features"
        )
    ...
    def create_time_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        features["is_weekend"] = (features["day_of_week"] >= 5).astype(
            int
        )  # Crypto trades 24/7 but this can be useful
    ...
    def create_all_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        key_features_for_lags = all_features[
            ["close", "volume", "price_change", "RSI_14", "MACD"]
        ].copy()
        ...
        self.logger.info(
            f"Feature engineering completed. Created {len(all_features.columns)} total features"
        )
        return all_features
    ...
    def select_features(
        ...
        if method == "mutual_info":
            ...
            selector = SelectKBest(
                score_func=mutual_info_classif, k=min(top_k, X_clean.shape[1])
            )
    ...
    def transform_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.selected_features:
            available_features = [
                f for f in self.selected_features if f in X_clean.columns
            ]
    ...
    def memory_usage_report(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        self.logger.info(
            f"Memory usage: {report['total_memory_mb']:.2f} MB for {report['num_features']} features"
        )
        return report
